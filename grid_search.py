...
def make_grid_hb_and_nag(L, nb_points):
...
def grid_search_momentum(mu, L, nb_points, max_length, alg):
    beta_list, alpha_list = make_grid_hb_and_nag(L=L, nb_points=nb_points)
    cv_list = []
    cycle_list = []

    if alg == "hb":
        find_cv = wc_heavy_ball_momentum
        find_cycle = cycle_heavy_ball_momentum
    elif alg == "nag":
        find_cv = wc_accelerated_gradient_strongly_convex
        find_cycle = cycle_accelerated_gradient_strongly_convex
    else:
        raise Exception("Wrong alg")
    ...
    for i in tqdm(range(len(beta_list))):
        ...
        for n in range(3, max_length + 1):
            if cv is False and cycle is False:
                cv = find_cv(mu=mu, L=L, alpha=alpha, beta=beta, n=n, verbose=-1)
            if cv is False and cycle is False:
                cycle = find_cycle(mu=mu, L=L, alpha=alpha, beta=beta, n=n, threshold=10 ** -3, verbose=-1)
        cv_list.append(cv)
...
def get_graphic(mu, L, nb_points, max_length, alg):
    ...
    if alg == "hb":
        beta_list, alpha_list, cv_list, cycle_list = grid_search_momentum(mu, L, nb_points, max_length, alg="hb")
        ...
        beta_list, alpha_list, cv_list, cycle_list = grid_search_momentum(mu, L, nb_points, max_length, alg="nag")
        plt.figure(figsize=(15, 9))
        plt.plot(alpha_list[cv_list], beta_list[cv_list], '.g')
        plt.plot(alpha_list[cycle_list], beta_list[cycle_list], '.r')
        plt.plot(alpha_list[~cv_list & ~cycle_list], beta_list[~cv_list & ~cycle_list], '.k')
...
if __name__ == "__main__":
    ...
    def get_the_right_graphic(alg):
        get_graphic(mu=.1, L=1., nb_points=10, max_length=8, alg=alg)
